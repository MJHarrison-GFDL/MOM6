...
subroutine build_hycom_2d_column(CS, rmask, remapCS, eqn_of_state, nz, depth, h, T, S, p_col, &
  ...
  real :: wt
  real, dimension(CS%nk+1) :: target_dens
  real, dimension(CS%nk) :: coord_res
  ...
  h_col_new(:)=0.0;z_col_new(:)=0.0;target_dens(:)=0.0
  do k=1,CS%ng
    wt=rmask(k)
    if (wt>0.0) target_dens(:) = target_dens(:) + rmask(k)*CS%target_density(:,k)
  enddo


  call build_and_interpolate_grid(CS%interp_CS, rho_col, nz, h(:), z_col, &
      target_dens, CS%nk, h1_col_new, z1_col_new, h_neglect, h_neglect_edge)
  ...
  do k=1,CS%ng
    wt=rmask(k)
    if (wt>0.0) coord_res(:) = coord_res(:) + rmask(k)*CS%coordinateResolution(:,k)
  enddo


  do k = 2, CS%nk+1
    nominal_z = nominal_z + (z_scale * (coord_res(k-1) * stretching))
